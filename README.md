...
Do Not Rename Folders: Never rename the root directories (obe_assess, frontend, etc.).

The "Apps" Rule: All backend features (like Assessment Marking) must be created as Django Apps inside the obe_assess/ folder. Do not create separate root folders for your work.

Branching: Never push directly to main. Create a branch first:

git checkout -b feature/assessment-marking

Syncing: Always run git pull origin main before starting work to ensure your local structure matches the server.
...
Configure PostgreSQL: Create a database named obe_assess_db and a user obe_user in pgAdmin/psql. Then run:
...
python manage.py startapp assessment_marking

Add 'assessment_marking', to INSTALLED_APPS in obe_assess/settings.py.
